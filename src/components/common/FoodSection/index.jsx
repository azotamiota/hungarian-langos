...
function index({isBiggerThanMobile, products, name}) {
  ...
  const sectionId = splitInHalf(name).join``.toLowerCase().replace(/\s/g, '')

  return (
    <div id={isBiggerThanMobile ? "street-food" : 'false'} className='products'>
      <header id={sectionId}><span className='red-letters'>{splitInHalf(name)[0]}</span><span className='green-letters'>{splitInHalf(name)[1]}</span></header>
